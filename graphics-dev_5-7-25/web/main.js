...
function setupTabNavigation() {
  ...
  // Clear any existing event listeners (requires modern browsers)
  ...
  // Set initial state - ensure home tab is visible
  const homeTab = document.getElementById("home-tab")
  if (homeTab) homeTab.style.display = "block"

  // Attach event listeners ONCE per button
  freshButtons.forEach((button) => {
    // Touch events - add OUTSIDE the click handler
    ...
    button.addEventListener("touchend", function (e) {
      ...
      // Instead of calling click(), handle tab switching directly
  ...
  function switchToTab(tabId) {
    ...
    // Update any SVG icons
    updateTabIcons()
  }

  // Handle SVG icon updates
  function updateTabIcons() {
    freshButtons.forEach((button) => {
      const iconElement = button.querySelector(".tab-icon img")
      if (iconElement) {
        const baseIconPath = iconElement.getAttribute("data-icon-base")
        const activeIconPath = iconElement.getAttribute("data-icon-active")

        if (baseIconPath && activeIconPath) {
          iconElement.src = button.classList.contains("active")
            ? activeIconPath
            : baseIconPath
        }
      }
...
// IMPORTANT: Remove duplicate setupTabNavigation() calls
// Call it only ONCE at the end
document.addEventListener("DOMContentLoaded", function () {
  console.log("DOM loaded, setting up tabs")
  setupTabNavigation()
})

// Ensure function runs once DOM is loaded
document.addEventListener("DOMContentLoaded", function () {
  console.log("DOM loaded, setting up tabs")
  setupTabNavigation()

  // Also set it up after a slight delay as a fallback
...
function updateButtonStates(signalId, state) {
  ...
  for (let i = 0; i < buttons.length; i++) {
    ...
    if (btn.classList.contains("btn-pill--small")) {
      ...
      btn.classList.remove("btn-pill--long--active", "btn-pill--long--inactive")
      btn.classList.add(
